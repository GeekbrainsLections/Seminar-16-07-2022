...
Для создание нового "сохранения" испольузется команда *git commit*. Используется она следующим образом: в терминале с папкой-репозиторием пишется *git commit -m <сообщение к коммиту>*. Сообщение к коммиту писать **ОБЯЗАТЕЛЬНО**!!! Для одновременного сохранения репозитория и создания коммита используется комманда *git commit -am <сообщение к коммиту>*
