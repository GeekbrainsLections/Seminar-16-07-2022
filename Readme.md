...
Для добавления файла к коммиту используется команда *git add*. Пишется она следующим образом *git add <имя файла>* в терминале в папке с созданным репозиторием. Так же можно использовать команду *commit -am "Сообщенеи к коммиту", эта команда выполнит добавление файла к коммиту и создаст новый коммит.
...
## Журнал изменений
Для просмотра журнала измений используется команда *git log*. Для этого в терминале в папке с репозиторием достаточно написать *git log*. Для просмотра "дерева" изменений, с наглядным отображением веток репозитория изспульзуется команда *git log --graph*.
